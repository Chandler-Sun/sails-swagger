...
const Transformer = {
    ...
    getDefinitions(sails) {
        let definitions = _.transform(sails.models, (definitions, model, modelName) => {
           let requiredProps = []
           let props = Transformer.getDefinitionProperties(model,requiredProps)
    ...
    getDefinitionProperties(model,requiredProps) {

        return _.mapValues(model.attributes, (attrDescObj, attrName) => {

            let mappedDef = attrDescObj.model && sails.config.blueprints.populate ? { '$ref': Transformer.generateDefinitionReference(attrDescObj.model) } : _.clone(Spec.getPropertyType(attrDescObj))
